...
import { amenities, boundary, market, qualifiedAddress } from "./market";
...
const storageKey = "house-decisions";
const compareStorageKey = "house-compare-v1";
const migrationKey = "house-decisions-cloud-migration-v1";
const outboxKey = "house-decisions-outbox-v1";
...
function MapView({ houses }: { houses: House[] }) {
  ...
  const resetMap=()=>{setCenter(market.center);setZoom(13)};
  return <div className="map-layout"><div className="area-map">
    <div className="map-toolbar"><div><strong>{market.city} terrain map</strong><span>{loading?"Locating homes…":`${pointCount} of ${houses.length} exact-address markers located`}</span></div><div className="map-legend"><span className="legend-house">Price pin</span><span className="legend-amenity">Amenity</span></div></div>
    <div ref={mapRef} className={`terrain-map ${dragRef.current?"dragging":""}`} role="application" aria-label={`Interactive terrain map of matching houses and nearby ${market.city} amenities`}
      ...
      <div className="boundary-label">{market.label}</div>
...
export default function HouseDashboard({ houses }: { houses: House[] }) {
  ...
  const requestResearch = async (house: House) => {
    const record: ResearchRecord = { houseId: house.id, address: qualifiedAddress(house.address), status: "requested", summary: "", sourcesChecked: "", checkedAt: "", updatedAt: new Date().toISOString() };
  ...
  const filterOptions: FilterOption[] = [
    { label: "Under $400K", group: "price" }, { label: "$400–499K", group: "price" }, { label: "$500–599K", group: "price" }, { label: "$600K+", group: "price" },
  ...
  const visible = useMemo(() => {
    const filtered = houses.filter((house) => {
      ...
      const matchesPrice = priceFilters.length === 0 || priceFilters.some((item) => item === "Under $400K" ? house.price < 400000 : item === "$400–499K" ? house.price >= 400000 && house.price < 500000 : item === "$500–599K" ? house.price >= 500000 && house.price < 600000 : house.price >= 600000);
  ...
  return <main>
    ...
    {negotiationHouse&&<NegotiationSimulator house={negotiationHouse} backgroundResearch={research[negotiationHouse.id]?.summary??""} onClose={()=>setNegotiationHouse(null)}/>}
      <section className="hero" id="top"><div><p className="eyebrow">{market.label} · DETACHED SINGLE-FAMILY ONLY</p><h1>Your house hunt,<br/><em>rebuilt.</em></h1><p className="hero-copy">Full multi-page public inventory, deduplicated by address. Twin homes, townhomes and condos are excluded.</p></div><div className="sync-card"><span className="pulse"/><div><strong>Two-hour watch</strong><p>Checks every results page for new listings, status changes, prices, facade photos, construction year, and open houses.</p><small>Inventory rebuilt Sep 2, 2026<br/><span>Last refreshed Sep 2, 2026 · 2:00 PM CDT</span><b className="new-count">{newlyAdded} newly added since last refresh</b></small></div></div></section>
    <section className="metrics" aria-label="House hunt summary"><div><span>{houses.length}</span><p>Total matches</p></div><div><span>{fourHundreds}</span><p>Homes from $400–499K</p></div><div><span>{scheduled}</span><p>Open houses flagged</p></div><div><span>{interested}</span><p>Marked interested</p></div></section>
    ...
    <footer><span>House Hunt · {market.city}</span><p>Payment estimates are planning figures, not a Loan Estimate. Exact property tax, homeowners insurance, PMI, HOA, points, closing costs, and final rate may change the payment. Verify listing details with the named source.</p></footer>
  </main>;
}
